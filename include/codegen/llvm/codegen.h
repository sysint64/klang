...
public:
	...
	virtual void emit() override {
		...
		emitIR ("\n; Type declares\n" ); for (auto const &IR : blockDType) emitIR (IR->emit);
		emitIR ("\n; Global vars\n"   ); for (auto const &IR : blockHead ) emitIR (IR->emit);
		emitIR ("\n; Funcs declares\n"); for (auto const &IR : blockEnd  ) emitIR (IR->emit);
		emitIR ("\n; Implementation\n"); for (auto const &IR : blockBody ) emitIR (IR->emit);
